...
describe("DAO Functions", function () {
  it("It should adjust VADER using Council", async () => {
    ...
    assert.equal(await vader.minting(), false)
    await dao.newActionProposal("MINTING")
    await dao.voteForProposal()
    await setNextBlockTimestamp(ts0 + 3 * 15)
    await dao.executeProposal()
    assert.equal(await vader.minting(), true)
  })

  it("It should prepare VAULT weighting", async () => {
